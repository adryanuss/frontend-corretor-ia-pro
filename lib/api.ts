...
export interface GenerateRequest { propertyType: string; bedrooms?: number; bathrooms?: number; area?: number; location: string; price?: number; highlights: string; tone?: 'formal' | 'casual' | 'luxury'; }
...
export const authAPI = {
  async register(name: string, email: string, password: string): Promise<AuthResponse> {
    const res = await fetch(`${API_URL}/api/auth/register`, { method: 'POST', headers: getHeaders(), body: JSON.stringify({ name, email, password }) });
    return handleResponse<AuthResponse>(res);
  },
  async login(email: string, password: string): Promise<AuthResponse> {
    const res = await fetch(`${API_URL}/api/auth/login`, { method: 'POST', headers: getHeaders(), body: JSON.stringify({ email, password }) });
    return handleResponse<AuthResponse>(res);
  },
};
export const listingsAPI = {
  async generate(data: GenerateRequest): Promise<{ listing: Listing; usage: UsageInfo }> {
    const res = await fetch(`${API_URL}/api/listings/generate`, { method: 'POST', headers: getHeaders(true), body: JSON.stringify(data) });
